...
const jobFormSchema = z.object({
    jobTitle: z.string().min(2, "El título es obligatorio"),
    ...
    skills: z.string().min(2, "Las habilidades son obligatorias"),
...
const Step1 = ({ data, setData,formErrors }: Step1Props) => (
    <div>
        ...
        <div className="space-y-6">
            <div>
                ...
                {formErrors.jobTitle && <p className="text-red-600 text-xs">{formErrors.jobTitle}</p>}
            </div>
            <div>
...
export default function Page() {
    ...
    const validateStep = () => {
        try {
            ...
            switch (currentStep) {
                case 1:
                    schema = jobFormSchema.pick({ jobTitle: true, company: true, location: true });
                    break;
                case 2:
                    schema = jobFormSchema.pick({ description: true });
                    break;
                case 3:
                    schema = jobFormSchema.pick({ skills: true });
                    break;
                default:
                    schema = null;
            }
            if (schema) schema.parse(formData);
            setFormErrors({});
            return true;
        } catch (e) {
    ...
    const isNextDisabled = () => {
        try {
            switch (currentStep) {
                case 1:
                    jobFormSchema.pick({ jobTitle: true, company: true, location: true }).parse(formData);
                    break;
                case 2:
                    jobFormSchema.pick({ description: true }).parse(formData);
                    break;
                case 3:
                    jobFormSchema.pick({ skills: true }).parse(formData);
                    break;
                default:
                    return false;
            }
